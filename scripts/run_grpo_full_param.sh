...
FULL_GRPO_RUN_TIMESTAMP="${FULL_GRPO_RUN_TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}"

FULL_GRPO_MODEL_PATH="${FULL_GRPO_MODEL_PATH:-/root/autodl-tmp/outputs/game24-sft-full/global_step_363/huggingface}"
...
FULL_GRPO_OUTPUT_DIR="${FULL_GRPO_OUTPUT_DIR:-/root/autodl-tmp/outputs/game24-grpo-full-param-${FULL_GRPO_RUN_TIMESTAMP}}"
...
FULL_GRPO_LOG_FILE="${FULL_GRPO_LOG_FILE:-}"
...
FULL_GRPO_TOTAL_TRAINING_STEPS="${FULL_GRPO_TOTAL_TRAINING_STEPS:-400}"
FULL_GRPO_SAVE_FREQ="${FULL_GRPO_SAVE_FREQ:-25}"
FULL_GRPO_VAL_FREQ="${FULL_GRPO_VAL_FREQ:-${FULL_GRPO_TEST_FREQ:-25}}"
FULL_GRPO_VAL_BEFORE_TRAIN="${FULL_GRPO_VAL_BEFORE_TRAIN:-true}"
FULL_GRPO_MAX_ACTOR_CKPT_TO_KEEP="${FULL_GRPO_MAX_ACTOR_CKPT_TO_KEEP:-1}"
...
FULL_GRPO_PPO_EPOCHS="${FULL_GRPO_PPO_EPOCHS:-1}"
...
FULL_GRPO_RESUME_MODE="${FULL_GRPO_RESUME_MODE:-disable}"
FULL_GRPO_RESUME_FROM_PATH="${FULL_GRPO_RESUME_FROM_PATH:-}"

if [[ -n "${FULL_GRPO_LOG_FILE}" ]]; then
  mkdir -p "$(dirname "${FULL_GRPO_LOG_FILE}")"
  exec > >(tee -a "${FULL_GRPO_LOG_FILE}") 2>&1
fi
...
Example:
  FULL_GRPO_MODEL_PATH=/root/autodl-tmp/outputs/game24-sft-full/global_step_363/huggingface bash scripts/run_grpo_full_param.sh
ERROR
  exit 2
fi
...
require_positive_integer "FULL_GRPO_VAL_FREQ" "${FULL_GRPO_VAL_FREQ}"
require_positive_integer "FULL_GRPO_MAX_ACTOR_CKPT_TO_KEEP" "${FULL_GRPO_MAX_ACTOR_CKPT_TO_KEEP}"
...
require_positive_integer "FULL_GRPO_PPO_EPOCHS" "${FULL_GRPO_PPO_EPOCHS}"
...
case "${FULL_GRPO_RESUME_MODE}" in
  disable) ;;
  resume_path)
    if [[ -z "${FULL_GRPO_RESUME_FROM_PATH}" ]]; then
      fail "FULL_GRPO_RESUME_FROM_PATH must be set when FULL_GRPO_RESUME_MODE=resume_path"
    fi
    ;;
  *)
    fail "FULL_GRPO_RESUME_MODE must be disable or resume_path, got '${FULL_GRPO_RESUME_MODE}'"
    ;;
esac

RESUME_ENABLED=false
if [[ "${FULL_GRPO_RESUME_MODE}" == "resume_path" ]]; then
  RESUME_ENABLED=true
fi
...
print_disk_report() {
  echo "Disk report before full-parameter GRPO"
  echo "  Command: df -h /root/autodl-tmp"
  if [[ -e /root/autodl-tmp ]]; then
    df -h /root/autodl-tmp
  else
    echo "  /root/autodl-tmp is not present on this machine; skipping df for that path."
  fi
  echo "  Command: du -sh \"${FULL_GRPO_OUTPUT_DIR}\""
  if [[ -e "${FULL_GRPO_OUTPUT_DIR}" ]]; then
    du -sh "${FULL_GRPO_OUTPUT_DIR}"
  else
    echo "  ${FULL_GRPO_OUTPUT_DIR} does not exist yet; du skipped."
  fi
}

list_remaining_checkpoints() {
  echo "Remaining global_step directories under ${FULL_GRPO_OUTPUT_DIR}:"
  if [[ -d "${FULL_GRPO_OUTPUT_DIR}" ]]; then
    find "${FULL_GRPO_OUTPUT_DIR}" -maxdepth 1 -type d -name 'global_step_*' -printf '  %f\n' | sort -V
  else
...
${PARAMETER_SUMMARY}
  ...
  FULL_GRPO_PPO_EPOCHS=${FULL_GRPO_PPO_EPOCHS}
  ...
  FULL_GRPO_VAL_FREQ=${FULL_GRPO_VAL_FREQ}
  FULL_GRPO_VAL_BEFORE_TRAIN=${FULL_GRPO_VAL_BEFORE_TRAIN}
  FULL_GRPO_MAX_ACTOR_CKPT_TO_KEEP=${FULL_GRPO_MAX_ACTOR_CKPT_TO_KEEP}
  ...
  FULL_GRPO_RESUME_MODE=${FULL_GRPO_RESUME_MODE}
  FULL_GRPO_RESUME_FROM_PATH=${FULL_GRPO_RESUME_FROM_PATH}
  CHECKPOINT_SAVE_FREQUENCY=${FULL_GRPO_SAVE_FREQ}
  MAX_ACTOR_CHECKPOINTS_TO_KEEP=${FULL_GRPO_MAX_ACTOR_CKPT_TO_KEEP}
  VERL_RETENTION_CONFIG=trainer.max_actor_ckpt_to_keep=${FULL_GRPO_MAX_ACTOR_CKPT_TO_KEEP}
  VERL_RETENTION_BEHAVIOR=save new global_step_N/actor first, then remove older tracked actor paths
  EXPECTED_RETENTION_LOG=Checkpoint manager remove previous save local path: .../global_step_N/actor
CONFIG
...
python -m verl.trainer.main_ppo \
  ...
  actor_rollout_ref.actor.ppo_epochs="${FULL_GRPO_PPO_EPOCHS}" \
  ...
  trainer.test_freq="${FULL_GRPO_VAL_FREQ}" \
  trainer.val_before_train="${FULL_GRPO_VAL_BEFORE_TRAIN}" \
  trainer.max_actor_ckpt_to_keep="${FULL_GRPO_MAX_ACTOR_CKPT_TO_KEEP}" \
  trainer.total_training_steps="${FULL_GRPO_TOTAL_TRAINING_STEPS}" \
  trainer.total_epochs="${FULL_GRPO_TOTAL_EPOCHS}" \
  trainer.resume_mode="${FULL_GRPO_RESUME_MODE}" \
  trainer.resume_from_path="${FULL_GRPO_RESUME_FROM_PATH}"
