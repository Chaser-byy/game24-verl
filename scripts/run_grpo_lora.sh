...
GRPO_TRAIN_BATCH_SIZE="${GRPO_TRAIN_BATCH_SIZE:-4}"
...
GRPO_TOTAL_TRAINING_STEPS="${GRPO_TOTAL_TRAINING_STEPS:-5}"
...
GRPO_ATTN_IMPLEMENTATION="${GRPO_ATTN_IMPLEMENTATION:-sdpa}"
GRPO_USE_REMOVE_PADDING="${GRPO_USE_REMOVE_PADDING:-false}"
...
Game24 verl GRPO LoRA configuration after SFT warm start
  ...
  GRPO_ATTN_IMPLEMENTATION=${GRPO_ATTN_IMPLEMENTATION}
  GRPO_USE_REMOVE_PADDING=${GRPO_USE_REMOVE_PADDING}
CONFIG
...
python -m verl.trainer.main_ppo \
  ...
  ++actor_rollout_ref.model.override_config.attn_implementation="${GRPO_ATTN_IMPLEMENTATION}" \
  actor_rollout_ref.model.use_remove_padding="${GRPO_USE_REMOVE_PADDING}" \
